...
{
    return m_raw.m_reqRaw.m_requestHeaders;
}
...
{
    return m_raw.m_reqRaw.m_requestHeaders.keys();
}

bool IHeaderJar::containRequestHeaderKey(IStringView key) const
{
    return m_raw.m_reqRaw.m_requestHeaders.contains(key);
}
...
{
    return m_raw.m_reqRaw.m_requestHeaders.value(view);
}
...
{
    return m_raw.m_reqRaw.m_requestHeaders.values(key);
}
...
{
    return m_raw.m_respRaw.headers;
}
...
{
    return m_raw.m_respRaw.headers.keys();
}

bool IHeaderJar::containResponseHeaderKey(const QString &key) const
{
    auto range = m_raw.m_respRaw.headers.equal_range(key);
...
{
    m_raw.m_respRaw.headers.insertMulti(std::move(key), std::move(value));
}

void IHeaderJar::addResponseHeader(QString key, const QStringList &values)
{
    for(const auto& value : values){
        m_raw.m_respRaw.headers.insertMulti(std::move(key), value);
    }
}
...
void IHeaderJar::setResponseHeader(QString key, QString value)
{
    ...
    m_raw.m_respRaw.headers.insert(std::move(key), std::move(value));
}
...
void IHeaderJar::setResponseHeader(QString key, const QStringList &values)
{
    deleteReponseHeader(key);
    for(const auto& value : values){
        m_raw.m_respRaw.headers.insert(key, value);
    }
}

void IHeaderJar::deleteReponseHeader(const QString &key)
{
    auto& headers = m_raw.m_respRaw.headers;
