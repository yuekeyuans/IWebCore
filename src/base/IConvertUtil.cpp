...
QByteArray IConvertUtil::urlEncode(const QString &rawUrl)
{
    QByteArray str = rawUrl.toUtf8();
    return urlEncode(str);
}

QByteArray IConvertUtil::urlEncode(const QByteArray &str)
{
    QByteArray strTemp;
...
QByteArray IConvertUtil::urlDecode(const QString &rawUrl)
{
    QByteArray str = rawUrl.toLocal8Bit();
    return urlDecode(str);
}

QByteArray IConvertUtil::urlDecode(const QByteArray &str)
{
    QByteArray strTemp;
    uint length = str.length();
    for (uint i = 0; i < length; i++)
...
QString IConvertUtil::pathEncode(const QString &rawPath)
{
    QByteArray strTemp;
